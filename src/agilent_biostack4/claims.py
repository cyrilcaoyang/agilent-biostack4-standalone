...
Lifecycle (per ``docs/STATUS_SPEC.md`` §5)::
...
``agilent-plateloc-server``'s ``claims.py`` (the closest v1.1 template on
this PC). Only this module docstring and the ``from .models import ...``
line differ; the claim models are vendored identically in ``models.py``.
"""
